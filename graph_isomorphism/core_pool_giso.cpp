...
int main(int argc,char**argv){
	...
	for(int i = 0; i<g1[0].size() && !giso;i++){
		for(int j=0;j<g2[0].size() && !giso;j++){
			...
			int current_available_thread = thread_pool.front();
			thread_pool.pop();
			args[arg_index].tnum=current_available_thread;
			pthread_create(&threads[current_available_thread],NULL,parallel_isomorphism_check_util,&args[arg_index]);
